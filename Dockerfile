...
RUN apt-get update && apt-get -y upgrade && \
    apt-get install --no-install-recommends -y ca-certificates sudo git curl bash-completion vim-tiny supervisor ghostscript imagemagick libgdal-dev libimage-exiftool-perl make g++ python qgis python-qgis qgis-plugin-grass
